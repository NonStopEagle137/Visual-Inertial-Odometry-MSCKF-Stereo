...
# %% Import IMU dataset
#  CSV imu file
fileName = 'MachineHall01_reduced'
dirname = f'../dataset/{fileName}/imu0/'
...
main_data_dir = f"../dataset/{fileName}/"
...
while True:
    ...
    pose.append((nominal_state[2], nominal_state[0], nominal_state[1], nominal_state[3].copy(), nominal_state[5], nominal_state[4]))
...
plt.legend()
plt.savefig('./rpy_translation.png', dpi = 500)
plt.show()

#%%

plt.figure()
plt.plot(velocity[:, 0], label='vx', c='r')
plt.plot(velocity[:, 1], label='vy', c='b')
plt.plot(velocity[:, 2], label='vz', c='g')
...
plt.ylabel('Linear Accelerations')
#plt.title('Accelerometer Bias')
plt.legend()
plt.savefig('./lin_acc.png', dpi = 500)
plt.show()

#%%
plt.figure()
plt.plot(g_term[:, 0], label='gx')
plt.plot(g_term[:, 1], label='gy')
plt.plot(g_term[:, 2], label='gz')
...
#%%
plt.figure()
plt.plot(w_b_term[:, 0], label='wx')
plt.plot(w_b_term[:, 1], label='wy')
plt.plot(w_b_term[:, 2], label='wz')
plt.ylabel('radians per second squared')
plt.title('w_b')
plt.legend()
plt.show()
